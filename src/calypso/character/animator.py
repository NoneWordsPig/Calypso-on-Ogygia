...
class Animator:
    def __init__(self, manifest='assets/calypso/manifest.json', fps=8.0, run_factor=1.8):
        ...
        self.fps=float(fps); self.run_factor=float(run_factor); self.state='idle_down'; self.frame=0; self._elapsed=0.0
    def _entry(self):
    ...
    def tick(self, dt, running=False):
        ...
        step=int(self._elapsed*self.fps)
        if step: self.frame=(self.frame+step)%max(1,self._entry().get('count',4)); self._elapsed-=step/self.fps
        return self.frame
    def frame_path(self):
